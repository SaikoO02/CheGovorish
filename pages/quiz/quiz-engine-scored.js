function runScoredQuiz() {
  const quiz = window.QUIZ_DATA || [];
  const results = window.QUIZ_RESULTS || [];
  ...
  function next() {
    ...
    if (scoreEl) scoreEl.textContent = `Уровень ${bestIndex + 1}/${results.length}`;

    const descEl = document.getElementById("quiz-finish-desc");
    if (descEl) descEl.textContent = results[bestIndex] || "";
  }
...
document.addEventListener("DOMContentLoaded", runScoredQuiz);
